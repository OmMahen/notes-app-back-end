...
class NoteService {
  ...
  async addNote({
    ...
    // hapus cache, agar tetap up-to-date
  ...
  async getNotes(owner) {
    try {
      // coba ambil notes dari cache dulu
      const result = await this._cacheService.get(`notes:${owner}`);
      return JSON.parse(result); // karena masih string, di ubah ke JSON dulu
    } catch (error) {
      // jika gagal, ambil dari database
      const query = {
